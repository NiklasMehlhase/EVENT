...
public class GrabVR : MonoBehaviour {
	...
	void Start () {
		this.leftRight = this.name.Equals ("LeftControllerAnchor");
    ...
    public HashSet<GrabObject> GetGrabbedObjects()
    {
	...
	void Update () {
		...
		bool curAxis = false;
		if(this.leftRight) {
            curAxis = OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger)>0.4f;//Input.GetAxisRaw ("LeftControllerGrip") > 0.0f;
			/*if (!curAxis) {
				curAxis = Input.GetAxisRaw ("LeftControllerTrigger") > 0.0f;
			}*/
		}
		else {
			curAxis = OVRInput.Get(OVRInput.Axis1D.SecondaryHandTrigger) > 0.4f;//Input.GetAxisRaw ("RightControllerGrip") > 0.0f;		
